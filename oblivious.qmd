...
In "Oblivious data for fairness with kernels" [@2021-oblivious], Steffen Grünewälder and Azadeh Khaleghi consider the prediction of an outcome $y$ from nonsensitive information $x$ where there exists some sensitive information $s$ that may be correlated with $x$. Dependence between $x$ and $s$ is a fairness issue if one wants to guarantee that predictions be independent from sensitive information $y\perp\!\!\!\perp s$. The authors restrict their discussion to kernel-based learning, that is the broad class of learning algorithms that, instead of relying directly on $(x_1...x_n)$, may be expressed so as to rely on a $n\times n$ positive semi-definite matrix $K_n$ summarizing the dependence between the observations.

In this context, they devise how to construct "oblivious" features $z=\phi(x)$ that both (a) guarantee the independence requirement $z \perp\!\!\!\perp s$ and (b) retain the information contained in $x$, in some maximal sense. After showing that a strict respect of both criteria is infeasible, they provide relaxed constraints and approximate solutions. The $z$ features so constructed are to be used in the construction of the kernel matrix in place of $x$. Such features may be released and used for prediction, without sharing any confidential data.
...
Imagine you have at our disposal data $(X,S)$ where the $X$'s are nonsensitive features but the $S$'s are sensitive features. The goal of the article is to devise a procedure that allows to generate a new random variable $Z$ that should ideally be 

- Independent from $S$ 
- As close to $X$ as possible, in some sense to be defined

We will see that later, the independence will be relaxed to a less binding constraint. This article particularly studies how to achieve this goal when we want to use kernel method.
