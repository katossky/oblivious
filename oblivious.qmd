...
The oblivious kernel matrix $\mathbb{O}$ is a n by n matrix containing the scalar products of the $\mathbf{z}$ ($\mathcal{O}_{ij} = \langle \mathbf{z}_i, \mathbf{z}_j\rangle$). Computing this matrix boils down to computing $\langle \mathbf{z}_i, \mathbf{z}_j\rangle \quad \forall i, j$. Using @eq-boldz, and given that we are doing estimations, we can write that: $$\mathbf{z}_i = \varphi(x_i) - \mathbb{E}_n\big[\varphi(x_i)\mid s_i\big] + \mathbb{E}\varphi(x_i)$$
So, the scalar product that we want to estimate is: 
$$\langle \varphi(x_i) - \mathbb{E}_n\big[\varphi(x_i)\mid s_i\big] + \mathbb{E}\varphi(x_i), \quad \varphi(x_j) - \mathbb{E}_n\big[\varphi(x_j)\mid s_j\big] + \mathbb{E}\varphi(x_j) \rangle$$
This scalar product can be dcomposed in 9 terms. 
In the article, the appendix E1 explains how each term can be boiled down to kernel evaluations, and provide a generic algorithm to do so (including a kernel $k(.,.)$ to be chosen). 
...
$$\tilde y \overset{\Delta}= \mathbb{1}(\tilde u\geqslant \tilde x_0) \; \mathbb{1}(\tilde x+\tilde s\geqslant \theta)$$

with $\tilde u \sim \mathcal{U}(1,4)$, or equivalently, $\mathbb{1}(\tilde u\geqslant \tilde x_0) \sim \mathcal{B}(\tilde x_0/4)$ \textbf{De mont point de vue il y a une erreur et ce devrait être $\mathbb{1}(\tilde u\leqslant \tilde x_0)$, ie plus ta note est élevé plus tu as de cahnce que $\tilde y_0 = 1$}
